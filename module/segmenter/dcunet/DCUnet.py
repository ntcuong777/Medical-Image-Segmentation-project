"""
    This source code is modified from https://github.com/Latterlig96/DCUnet/blob/main/model.py
    Some modifications:
        - Add DWSepConvBlock.
        - Add additional function parameters for using this network in Double-Unet style
"""
...
class DWSepConvBlock(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size = 3, stride = 1, padding = 1, activation=True):
        super(DWSepConvBlock, self).__init__()
        self.depthwise = nn.Conv2d(input_channels, input_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=input_channels)
...
class ConvTranspose(nn.Module): 
    ...
    def forward(self, 
                x1: torch.Tensor, 
                x2: torch.Tensor,
                x3: torch.Tensor = None, # Use in case of Double-Unet
                ) -> torch.Tensor:
...
class DcBlock(nn.Module): 
    def __init__(self,
        ...
        MyConvBlock = ConvBlock
        if depthwise_conv:
            MyConvBlock = DWSepConvBlock

        for i, filter in enumerate(self.filters): 
            if i == 0: 
                self.left_module.append(module=MyConvBlock(input_channels=self.input_channels + self.add_channels,
                                                         output_channels=filter,
                                                         kernel_size=kernel_size,
                                                         stride=stride,
                                                         padding=autopad(k=kernel_size, p=padding)))
                
                self.right_module.append(module=MyConvBlock(input_channels=self.input_channels + self.add_channels,
                                                          output_channels=filter,
                                                          kernel_size=kernel_size,
                                                          stride=stride,
                                                          padding=autopad(k=kernel_size, p=padding)))
            else: 
                self.left_module.append(module=MyConvBlock(input_channels=self.left_module[i-1].output_channels,
                                                         output_channels=filter,
                                                         kernel_size=kernel_size,
                                                         stride=stride,
                                                         padding=autopad(k=kernel_size, p=padding)))
                self.right_module.append(module=MyConvBlock(input_channels=self.right_module[i-1].output_channels,
                                                          output_channels=filter,
                                                          kernel_size=kernel_size,
                                                          stride=stride,
                                                          padding=autopad(k=kernel_size, p=padding)))
...
class ResPath(nn.Module):
    def __init__(self,
        ...
        self.conv1 = MyConvBlock(input_channels=self.input_channels,
        ...
        self.conv2 = MyConvBlock(input_channels=self.input_channels,
                               output_channels=self.output_channels,
                               kernel_size=(3, 3),
                               stride=(1, 1),
        ...
        for i in range(self.length-1):
            self.module.append(module=MyConvBlock(input_channels=self.output_channels,
                                                output_channels=self.output_channels,
                                                kernel_size=(1, 1),
                                                stride=(1, 1),
                                                activation=False,
...
class DcUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, other_net_decoder_out = None) -> torch.Tensor:
        """
            other_net_decoder_out: Outputs of another decoder output in case of Double-Unet
        """
        if other_net_decoder_out is None:
            ...
            up1 = self.up1(dc_block_5, dc_block_4, other_net_decoder_out[3])
            dc_block_6 = self.dc_block_6(up1)

            up2 = self.up2(dc_block_6, dc_block_3, other_net_decoder_out[2])
            dc_block_7 = self.dc_block_7(up2)

            up3 = self.up3(dc_block_7, dc_block_2, other_net_decoder_out[1])
            dc_block_8 = self.dc_block_8(up3)

            up4 = self.up4(dc_block_8, dc_block_1, other_net_decoder_out[0])
            dc_block_9 = self.dc_block_9(up4)
